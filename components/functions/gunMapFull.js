  ...
  return vl
    ...
    .encode(
      vl
        .y()
        .fieldN("state")
        .sort(vl.mean("percentage").order("descending"))
        .title("State"),
      vl.color().fieldQ("percentage"),
      vl.tooltip([
        vl.fieldN("state"),
        vl.fieldQ("percentage"),
        vl.fieldQ("guns/1M population/year"),
      ])
    );
}
...
function gunMap(data, usa, pst, gun) {
  return vl
    .layer(
      ...
      vl
        .markCircle({ stroke: "#5e5e5e", strokeWidth: 2 })
        .data(data)
        .encode(
...
function gunMapFull({ dataSet, options }) {
  const { g, stf, stp, usa, pst } = dataSet;
  ...
  const data = g
    ...
    .derive({
      percentage: (d) => (100 * d.count_guns) / d.total_guns,
      "guns/1M population/year": (d) => (d.count_guns / d.population) * 1000000,
    });
