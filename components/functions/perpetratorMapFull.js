...
function perpetratorRect(pst, field, yearStart, yearEnd) {
  return vl.markRect().data(pst)
    .transform(
      vl.filter('datum["year"] >= ' + yearStart + ' && datum["year"] <= ' + yearEnd)
    )
    .encode(
      vl.y().fieldN('state').sort(vl.mean(field).order('descending')).title("State"),
      vl.color().mean(field),
      vl.tooltip([vl.fieldN('state'), vl.mean(field)])
    )
}

function perpetratorMap(pst, field, yearStart, yearEnd) {
  const titles = {
  ...
  return vl.data(pst).layer(
    ...
    .encode(
      ...
      vl.color().mean(field).legend({titleLineHeight: 10}).scale({'scheme': schemes[field]}).title(titles[field]),
      vl.size().mean('perpetrators_per_1M').title(['Perpetrators/', '1M population/year']),
      vl.tooltip([vl.fieldN('state'), vl.fieldQ(field), vl.fieldQ('perpetrators_per_1M')])
    )
  )
...
function perpetratorMapFull({ dataSet, options }) {
  ...
  return vl.hconcat(
    perpetratorRect(pst, field, yearStart, yearEnd).height(500),
    perpetratorMap(pst, field, yearStart, yearEnd).height(500).width(720)
